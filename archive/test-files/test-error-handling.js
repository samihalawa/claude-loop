...
async function testErrorHandling() {
    ...
    try {
        ...
        const testPort1 = await aiConfig.allocatePort('test-error-1');
        const webUI3 = new WebUI(testPort1);
        await webUI3.start();
        
        const WebSocket = require('ws');
        try {
            const ws = new WebSocket(`ws://localhost:${testPort1}?token=invalid_token`);
        ...
        const testPort2 = await aiConfig.allocatePort('test-error-2');
        const webUI4 = new WebUI(testPort2);
        await webUI4.start();
        ...
        const testPort3 = await aiConfig.allocatePort('test-error-3');
        const webUI5 = new WebUI(testPort3);
        await webUI5.start();
        ...
        aiConfig.releasePort(testPort1, 'test-error-1');
        aiConfig.releasePort(testPort2, 'test-error-2');
        aiConfig.releasePort(testPort3, 'test-error-3');
