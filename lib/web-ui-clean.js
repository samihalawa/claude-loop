...
class WebUI {
    constructor(port = 3333) {
        ...
        // Simple WebSocket server - NO TOKENS AT ALL
    ...
    setupWebSocket() {
        this.wss.on('connection', (ws, req) => {
            ...
            // Send current session data immediately - NO TOKEN CHECK
            ...
            if (this.outputBuffer.length > 0) {
                ws.send(JSON.stringify({
                    type: 'output_batch',
                    data: this.outputBuffer.slice(-50)
                }));
            }
    ...
    handleWebSocketMessage(ws, data) {
        // NO TOKEN VALIDATION - DIRECT HANDLING
        switch (data.type) {
            case 'ping':
            ...
            const tryPort = (port) => {
                this.server.listen(port, (error) => {
                    if (error) {
                        ...
                        logger.info(chalk.green('✅ NO AUTHENTICATION REQUIRED - Direct access'));
                        resolve();
                    }
                });
            };
    ...
    // Utility methods for backward compatibility - NO TOKEN STUFF
    regenerateToken() {
        // No-op - NO TOKENS EVER
        logger.info(chalk.gray('No tokens needed - direct access'));
    }
