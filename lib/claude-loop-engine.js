...
function sanitizeCommand(command) {
    // Only allow known safe claude commands with strict validation
    ...
    if (/[;&|`$(){}[\]\\]/.test(command)) {
        console.warn(chalk.yellow(`⚠️  Suspicious command detected: ${command}`));
        return CLAUDE_LOOP.DEFAULT_CLAUDE_COMMAND;
    }
...
function createSecureTempFile(baseDir, prefix) {
    ...
    // Validate base directory is safe
    ...
    if (!isSafeDir) {
        console.warn(chalk.yellow(`⚠️  Unsafe temp directory, using current working directory: ${resolvedBaseDir}`));
        baseDir = process.cwd();
    }
    
    // Generate cryptographically secure filename
    const random = crypto.randomBytes(SECURITY.TOKEN_BYTES).toString(SECURITY.CRYPTO_ENCODING);
    const timestamp = Date.now();
    return path.join(baseDir, `${prefix}-${timestamp}-${random}.tmp`);
}
...
function validateFilePath(filePath) {
    ...
    try {
        ...
        // Ensure file is within current working directory or temp directory
...
function validatePromptContent(content) {
    ...
    // Check for potentially dangerous patterns in prompt content
    ...
    for (const pattern of dangerousPatterns) {
        if (pattern.test(content)) {
            console.warn(chalk.yellow(`⚠️  Potentially dangerous content detected in prompt`));
            return false;
        }
    }
    ...
    if (content.length > 100000) { // 100KB limit
        console.warn(chalk.yellow(`⚠️  Prompt content exceeds size limit: ${content.length} characters`));
        return false;
    }
...
class ClaudeLoopEngine {
    constructor(options = {}) {
        ...
        this.maxIterations = options.maxIterations || CLAUDE_LOOP.MAX_ITERATIONS;
    ...
    sanitizePromptContent(content) {
        ...
        // Remove any potential shell injection patterns
        ...
        if (sanitized.length !== content.length) {
            console.warn(chalk.yellow('⚠️  Prompt content was sanitized for security'));
        }
    ...
    async secureCleanupTempFile(filePath) {
        try {
            // Validate path is still in our tracked set
            if (!this.tempFiles.has(filePath)) {
                console.warn(chalk.yellow(`⚠️  Attempting to clean up untracked file: ${filePath}`));
                return false;
            }
            ...
            if (stats.size > 0) {
                ...
                const crypto = require('crypto');
                const randomData = crypto.randomBytes(Math.min(stats.size, 1024 * 1024)); // Limit to 1MB
            ...
            console.log(chalk.gray(`✓ Securely cleaned up temp file: ${path.basename(filePath)}`));
            return true;
        } catch (error) {
            console.log(chalk.gray(`Note: Could not clean up temp file ${path.basename(filePath)}: ${error.message}`));
            return false;
        }
    }
    ...
    async cleanup() {
        ...
        const cleanupPromises = Array.from(this.tempFiles).map(tempFile => 
            this.secureCleanupTempFile(tempFile)
        );
...
Use the Task tool aggressively. Create multiple parallel debugging agents. Fix every broken button, every non-working feature, every UI inconsistency. Make this repository work perfectly.`;
        ...
        try {
            ...
            const validatedPath = validateFilePath(tempPromptFile);
            this.tempFiles.add(validatedPath); // Track for cleanup
            console.log(chalk.gray(`Creating secure temp file: ${path.basename(validatedPath)}`));
            
            if (this.webUI) {
                this.webUI.addOutput(`Creating secure temp file: ${path.basename(validatedPath)}`, 'info');
            }
            
            try {
                // Create temp file with secure permissions and validate content
                const sanitizedPrompt = this.sanitizePromptContent(initialPrompt);
                await fs.writeFile(validatedPath, sanitizedPrompt, { 
                    mode: FILE_SYSTEM.TEMP_FILE_MODE,
                    flag: 'wx' // Create file exclusively, fail if exists
                });
                console.log(chalk.gray(`✓ Secure temp file created successfully`));
                if (this.webUI) {
                    this.webUI.addOutput('✓ Secure temp file created', 'success');
                }
                
                // Verify file was created and is secure
                const stats = await fs.stat(validatedPath);
                if ((stats.mode & parseInt('777', 8)) !== FILE_SYSTEM.TEMP_FILE_MODE) {
                    console.warn(chalk.yellow('⚠️  Temp file permissions may be insecure'));
                }
                console.log(chalk.gray(`✓ Temp file verified and accessible`));
            } catch (error) {
                console.error(chalk.red(`Failed to create secure temp file: ${error.message}`));
                if (this.webUI) {
                    this.webUI.addOutput(`❌ Failed to create secure temp file: ${error.message}`, 'error');
                }
                this.tempFiles.delete(validatedPath); // Remove from tracking if failed
                throw error;
            }
            
            // Use array-based arguments to prevent injection and validate content
            const promptContent = await fs.readFile(validatedPath, 'utf8');
            ...
            console.log(chalk.gray(`Max turns: ${CLAUDE_LOOP.MAX_TURNS_PER_ITERATION}`));
            ...
            if (this.webUI) {
                ...
                this.webUI.addOutput(`Max turns: ${CLAUDE_LOOP.MAX_TURNS_PER_ITERATION}`, 'info');
                this.webUI.addOutput('Mode: Full autonomy', 'info');
            }
            
            // Use safer file-based argument passing to prevent command injection
            const args = ['-f', validatedPath, '--max-turns', CLAUDE_LOOP.MAX_TURNS_PER_ITERATION.toString(), '--dangerously-skip-permissions'];
            ...
            const cleaned = await this.secureCleanupTempFile(validatedPath);
            if (cleaned && this.webUI) {
                this.webUI.addOutput('✓ Securely cleaned up temp file', 'success');
            } else if (!cleaned && this.webUI) {
                this.webUI.addOutput('Note: Could not clean up temp file', 'warning');
            }
            ...
            while (this.iteration < this.maxIterations) {
                ...
                console.log('━'.repeat(UI_CONFIG.SEPARATOR_LENGTH));
                ...
                await new Promise(resolve => setTimeout(resolve, TIMEOUTS.SESSION_SETTLE));
        ...
        if (this.webUI) {
            try {
                ...
                setTimeout(async () => {
                    await this.webUI.stop();
                }, TIMEOUTS.WEBUI_STOP_DELAY); // Stop after configured delay
            } catch (error) {
                console.error(chalk.red('Error stopping WebUI:'), error.message);
            }
        }
...
Fix any remaining issues.`
        ...
        try {
            ...
            try {
                await fs.writeFile(tempPromptFile, prompt, { mode: FILE_SYSTEM.TEMP_FILE_MODE });
            ...
            // Use spawn for secure execution with file-based approach
            const args = ['-c', '-f', tempPromptFile, '--max-turns', CLAUDE_LOOP.MAX_TURNS_PER_ITERATION.toString(), '--dangerously-skip-permissions'];
            const childProcess = spawn(this.claudeCommand, args, {
                cwd: this.repoPath,
    ...
    generateProgressBar(current, total, width = UI_CONFIG.PROGRESS_BAR_WIDTH) {
    ...
    formatElapsedTime(startTime) {
        ...
        const minutes = Math.floor(elapsed / TIME_CONSTANTS.MILLISECONDS_PER_MINUTE);
        const seconds = Math.floor((elapsed % TIME_CONSTANTS.MILLISECONDS_PER_MINUTE) / TIME_CONSTANTS.MILLISECONDS_PER_SECOND);
    ...
    async startUI() {
        try {
            ...
            const port = PORTS.WEBUI_DEFAULT;
