...
Start by exploring the codebase completely. Use Task tool to delegate complex analysis. Fix issues systematically.`;
        ...
        try {
            ...
            // Start the initial Claude session using correct CLI flags
            const allowedToolsStr = this.allowedTools.join(',');
            const startCmd = `${this.claudeCommand} -p "${initialPrompt.replace(/"/g, '\\"')}" --max-turns 30 --dangerously-skip-permissions --allowedTools "${allowedToolsStr}"`;
...
Fix any remaining issues.`
        ...
        try {
            // Use claude -c to continue with full permissions in non-interactive mode
            const continueCmd = `${this.claudeCommand} -c -p "${prompt.replace(/"/g, '\\"')}" --max-turns 30 --dangerously-skip-permissions`;
