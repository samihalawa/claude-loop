...
class ClaudeLoopEngine {
    ...
    async run() {
        ...
        const promptFile = path.join(this.repoPath, '.claude-loop-prompt.txt');
...
IMPORTANT: Start by running these commands to understand the project structure:
1. tree -I 'node_modules|.git|dist|build' | head -100
2. grep -r "app\\.get\\|app\\.post\\|router\\." --include="*.js" --include="*.ts" | head -50
3. grep -r "function\\|const.*=.*=>\\|class" --include="*.js" --include="*.ts" | head -50  
4. find . -name "*.json" -not -path "*/node_modules/*" | head -20
5. ls -la

Use the Task tool aggressively for complex debugging subtasks. Create multiple parallel Tasks.

Update Todos:
  ⎿  ☐ Get complete project structure overview (tree, endpoints, functions)
...
Start by exploring the codebase completely. Use Task tool to delegate complex analysis. Fix issues systematically.`;
        
        await fs.writeFile(promptFile, initialPrompt);
        ...
        try {
            ...
            // Start the initial Claude session using file input
            const startCmd = `${this.claudeCommand} --file "${promptFile}" --max-turns 30 --dangerously-skip-permissions --allowedTools ${this.allowedTools.map(t => `"${t}"`).join(' ')}`;
...
Fix any remaining issues.`
        ...
        try {
            // Write continuation prompt to file to avoid shell escaping issues
            const promptFile = path.join(this.repoPath, '.claude-loop-continue.txt');
            await fs.writeFile(promptFile, prompt);
            
            // Use claude -c to continue with full permissions using file input
            const continueCmd = `${this.claudeCommand} -c --file "${promptFile}" --max-turns 30 --dangerously-skip-permissions`;
