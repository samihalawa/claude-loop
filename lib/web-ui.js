...
class WebUI {
    ...
    setupMiddleware() {
        ...
        this.app.use((req, res, next) => {
            ...
            // Enhanced CSP to prevent XSS attacks while maintaining functionality
            res.setHeader('Content-Security-Policy', 
                "default-src 'self'; " +
                "script-src 'self' 'nonce-webui-scripts' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; " +
                "style-src 'self' 'nonce-webui-styles' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://fonts.googleapis.com; " +
...
<body>
    ...
    <script>
        ...
        function connect() {
            ...
            if (!token) {
                // Safely set error message without XSS vulnerability
                document.body.textContent = '';
                const errorContainer = document.createElement('div');
                errorContainer.style.cssText = 'text-align: center; padding: 50px; font-family: Inter, sans-serif; color: #e2e8f0; background: #0f1419;';
                
                const title = document.createElement('h1');
                title.textContent = 'Access Token Required';
                title.style.cssText = 'color: #f56565; margin-bottom: 20px;';
                
                const message = document.createElement('p');
                message.textContent = 'Please add ?token=YOUR_TOKEN to the URL';
                message.style.cssText = 'color: #8796a0;';
