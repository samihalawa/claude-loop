...
class WebUI {
    constructor(port = parseInt(process.env.WEBUI_PORT) || 3333) {
        ...
        // Generate session token for basic authentication
        this.sessionToken = crypto.randomBytes(32).toString('hex');
        console.log(chalk.cyan(`🔐 WebUI Access Token: ${this.sessionToken}`));
    ...
    setupWebSocket() {
        this.wss.on('connection', (ws, req) => {
            ...
            console.log(chalk.gray(`📱 Web UI client connected (${this.connectionCount}/${this.maxConnections})`));
            ...
            ws.on('close', () => {
                ...
                this.connectionCount--;
                console.log(chalk.gray(`📱 Web UI client disconnected (${this.connectionCount}/${this.maxConnections})`));
            });
...
<body>
    ...
    <script>
        ...
        function addOutputLine(entry) {
            ...
            line.innerHTML = \`
                <div class="output-line-icon">
                    <i class="\${iconClass}"></i>
                </div>
                <div class="output-line-content">
